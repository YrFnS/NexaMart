...
export function ShopPage() {
  ...
  const { selectedCategory, selectCategory, searchQuery, setSearchQuery } = useAppStore();
  ...
  }, [page, sort, categoryId, search, freeShipping, b2bOnly, onSale, priceRange]);
  ...
  if (freeShipping) {
    activeFilters.push({ type: 'freeShipping', label: t('freeShipping'), value: 'true' });
  }
  if (b2bOnly) {
  ...
  if (minRating > 0) {
    activeFilters.push({ type: 'rating', label: `${minRating}+ ★`, value: String(minRating) });
  }
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 min-w-0">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="newest">
                    <span className="flex items-center gap-2"><Clock className="size-3.5" /> {t('newest')}</span>
                  </SelectItem>
                  <SelectItem value="price-asc">
                    <span className="flex items-center gap-2"><ArrowUp className="size-3.5" /> {t('priceLowHigh')}</span>
                  </SelectItem>
                  <SelectItem value="price-desc">
                    <span className="flex items-center gap-2"><ArrowDown className="size-3.5" /> {t('priceHighLow')}</span>
                  </SelectItem>
                  <SelectItem value="rating">
                    <span className="flex items-center gap-2"><Star className="size-3.5" /> {t('bestRating')}</span>
                  </SelectItem>
                  <SelectItem value="popular">
                    <span className="flex items-center gap-2"><Heart className="size-3.5" /> {t('mostPopular')}</span>
                  </SelectItem>
                </SelectContent>
              </Select>
          ...
          {loading ? (
            ...
            <div className={`grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4 transition-opacity duration-300 ${isGridAnimating ? 'opacity-0' : 'opacity-100'}`}>
            ...
            <div className={`space-y-3 transition-opacity duration-300 ${isGridAnimating ? 'opacity-0' : 'opacity-100'}`}>
