...
export function ProductCard({
		...
		try {
			...
			if (Array.isArray(parsed)) {
				return parsed.filter(
					(img: string) => typeof img === "string" && img.startsWith("/"),
				);
			}
			return [];
		} catch {
	...
	const imageSrc = hasValidLocalImage
		? localImages[0]
		: getUnsplashImage(product.id);
		...
		<TooltipProvider delayDuration={300}>
			...
			>
				...
				<div className="relative aspect-square overflow-hidden bg-muted shrink-0">
					...
					{/* Badges */}
					<div
						className={`absolute top-2 ${isRTL ? "right-2" : "left-2"} flex flex-col gap-1 z-10`}
					>
						{product.isNew && (
					...
					{product.hasFreeShipping && (
						...
						>
							<Badge
								variant="secondary"
								className="text-[10px] px-1.5 py-0 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm shadow-sm"
							>
					...
					<div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 pointer-events-none">
						<div
							className="px-4 py-2 rounded-full bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm text-xs font-semibold shadow-lg border border-border/50"
							style={{ color: GOLD }}
						>
							...
							<TooltipTrigger asChild>
								<Button
									...
									className={`size-8 rounded-full shadow-lg bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm border border-white/20 ${
										isComparing
											? "text-white ring-2 hover:brightness-110"
											: "hover:bg-muted text-gray-600 dark:text-gray-400"
									}`}
									style={
										isComparing
											? {
													backgroundColor: GOLD,
													boxShadow: `0 0 0 2px ${GOLD}50`,
												}
											: undefined
									}
				...
				<div className="p-3 space-y-1 flex-1 flex flex-col">
					...
					{/* Product Name */}
					<h3
						className="text-sm font-medium leading-tight line-clamp-2 min-h-[2.5rem] transition-colors"
						style={{ "--tw-group-hover-color": GOLD } as React.CSSProperties}
					>
