...
export async function GET() {
  try {
    ...
    const listings = products.map((p) => ({
      ...
      reach: p.views * 3,
      clickThroughRate: p.views > 0 ? parseFloat(((p.soldCount / p.views) * 100).toFixed(1)) : 0,
      viewsData: [],
      similarAvgViews: 0,
      boostCost: 5.99,
    }));

    const avgCTR = listings.length > 0
      ? parseFloat((listings.reduce((sum, l) => sum + l.clickThroughRate, 0) / listings.length).toFixed(1))
      : 0;
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ error: 'Unknown action. Supported actions: boost, sale' }, { status: 400 });
  } catch (error) {
